...
from gesturevision.gesture_recognition.finger_geometry import is_finger_curled, is_finger_extended
from gesturevision.hand_tracking import landmarks as lm


class RockSignRule:
    gesture = GestureType.ROCK_SIGN

    def evaluate(self, hand: HandResult) -> float:
        points = hand.landmarks
        if not is_finger_extended(points, lm.INDEX_TIP, lm.INDEX_PIP):
            return 0.0
        if not is_finger_extended(points, lm.PINKY_TIP, lm.PINKY_PIP):
            return 0.0
        if not is_finger_curled(points, lm.MIDDLE_TIP, lm.MIDDLE_PIP):
            return 0.0
